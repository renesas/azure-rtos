                                    ...
                                    UCHAR *version, UINT version_length)
{
    ...
    if (update_state == NX_AZURE_IOT_ADU_AGENT_UPDATE_RECEIVED)
    {
        ...
        /* Received new update.  */
        LOG_TERMINAL("Received new update: Provider: %.*s; Name: %.*s, Version: %.*s\r\n",
...
VOID sample_adu_start(NX_AZURE_IOT_HUB_CLIENT *hub_client_ptr)
{
    if (adu_agent_started == NX_FALSE)
    {
        ...
        {
            LOG_TERMINAL("Failed on nx_azure_iot_adu_agent_start!\r\n");
            return;
        }

        LOG_TERMINAL("Manufacturer: %s, Model: %s, Installed Criteria: %s.\r\n", SAMPLE_DEVICE_MANUFACTURER, SAMPLE_DEVICE_MODEL, SAMPLE_DEVICE_INSTALLED_CRITERIA);
        ...
        {
            LOG_TERMINAL("Failed on nx_azure_iot_adu_agent_proxy_update_add!\r\n");
            return;
        }
