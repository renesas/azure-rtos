...
extern UINT gx_rx_graphics_driver_setup(GX_DISPLAY *display);
VOID clock_update();
...
/* replace malloc (used by Dave2D driver) to use ThreadX byte pool */
void *malloc(int size)
{
	return memory_allocate(size);
}

/** replace free (used by Dave2D driver) to use ThreadX byte pool */
void free(void *mem)
{
	memory_free(mem);
}
...
VOID  guix_setup(GX_UBYTE *first_unused_memory)
{
    ...
    gx_studio_display_configure(MAIN_DISPLAY, gx_rx_graphics_driver_setup,
        LANGUAGE_ENGLISH, MAIN_DISPLAY_THEME_1, &root);

    gx_canvas_memory_define(root->gx_window_root_canvas, (GX_COLOR *) working_frame, MAIN_DISPLAY_X_RESOLUTION * MAIN_DISPLAY_Y_RESOLUTION * 2);
