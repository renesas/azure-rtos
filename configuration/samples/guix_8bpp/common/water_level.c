...
VOID water_level_page_init()
{
    INT target_value = current_water_level;
    current_water_level = water_level_values[0];

    INT old_index = water_level_index_get(target_value);
    if (old_index)
    {
        gx_prompt_text_color_set((GX_PROMPT *)water_level_label_widgets[old_index], GX_COLOR_ID_AA_TEXT_NORMAL, GX_COLOR_ID_AA_TEXT_NORMAL, GX_COLOR_ID_AA_TEXT_NORMAL);
    }

     /* Attach water level page. */
    gx_widget_attach((GX_WIDGET *) &main_screen, &water_level_window);
    ...
    gx_slider_value_set((GX_SLIDER *)&water_level_window.water_level_window_water_level_slider,
     ...
     /* Start animation to move water level from 17 to current value. */
  
    water_level_animation_start(target_value);
}
...
VOID water_level_animation_start(INT target_value)
{
    ...
    water_level_animation_steps = target_value - current_water_level;
...
VOID water_level_animation_update(GX_WIDGET *widget)
{
    ...
    if (water_level_animation_steps > 0)
    {
        if (water_level_animation_steps >= 2)
        {
            ...
            new_value = current_water_level + 2;
        }
        else
        {
            water_level_animation_steps--;
            new_value = current_water_level + 1;
        }
    }
    else
    {
...
VOID water_level_value_set(int new_value)
{
    ...
    old_index = water_level_index_get(current_water_level);

    /* Set new water level value. */
    current_water_level = new_value;

    /* Get new water level index. */
    new_index = water_level_index_get(current_water_level);

    /* Transfer water level value to text. */
    gx_utility_ltoa(current_water_level, string_buffer, 10);
    ...
    if (old_index != new_index)
    {
        ...
        gx_prompt_text_color_set((GX_PROMPT *)water_level_label_widgets[new_index], GX_COLOR_ID_AA_TEXT_SELECTED, GX_COLOR_ID_AA_TEXT_SELECTED, GX_COLOR_ID_AA_TEXT_SELECTED);
    }
}
